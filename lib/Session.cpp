...
WId Session::windowId() const
{
    ...
    // With the Graphics View Framework TerminalDisplay does not have winId
    // because it instantiates QGraphicsWidget which is not a window in X11
    // sense. So, ID of the view or its parent is returned instead. For this to
    // work terminal widget should be added to scene before its
    // startShellProgram method is called (that results in a call to this
    // method). Otherwise scene() call below will return 0.

    if ( _views.count() == 0 )
       return 0;
    else
    {
        QGraphicsScene *scene = _views.first()->scene();
...
/**
 * Ported to Qt from KShell::homeDir
 */
static
QString homeDir( const QString &user )
{
    ...
    struct passwd *pwd = getpwnam(user.toAscii().data());
 ...
 *
 * Ported to Qt from KShell::tildeExpand
 *
...
/**
 * Checks that absolute path points to an executable program
 */
static
QString checkExe(const QString path)
{
    QFileInfo info(path);
    QString error;
...
/**
 * Finds the executable in the system path. Simplified, plain Qt version
 * of KGlobal::dirs()->findExe.
 *
 */
static
QString findExe(const QString& appname)
{
    QFileInfo info(appname);
    QString error;

    if (info.isAbsolute()) {
	return checkExe(appname);
    }

    QStringList pathList =
	QString::fromLocal8Bit(qgetenv("PATH")).split(QLatin1Char(':'));
    ...
    QString path;
    for (iter = pathList.constBegin(); iter != pathList.constEnd(); ++iter) {
	path = (*iter) + QDir::separator() + appname;
...
QString Session::checkProgram(const QString& program) const
{
  ...
  QString pexec = findExe(exec);
  if ( pexec.isEmpty() ) 
  {
    qDebug() << "Could not find binary: " << exec;
    return QString();
  }
...
void Session::terminalWarning(const QString& message)
{
    ...
    // 2 new lines a bit too much for a small display
...
void Session::run()
{
  ...
  {
      terminalWarning(QString("Could not find '%1'\n\rStarting '%2' instead.").arg(_program).arg(exec));

      _arguments = QStringList() << "-" + exec; // - for login
      _program = exec; // so that terminal widget could find out the change
  }
  // if none of the choices are available, print a warning
  else if (choice == CHOICE_COUNT)
  {
      terminalWarning("Could not find an interactive shell to start.");
      return;
  }
...
//    _shellProcess->setWorkingDirectory(QDir::homePath());
  ...
  _environment << QString("TERM=xterm");
  ...
  if (result < 0)
  {
      terminalWarning(QString("Could not start program '%1' with arguments '%2'.").arg(exec).arg(arguments.join(" ")));
    return;
  }
