...
/**
Graphics View Framework Port TODO:

- mapFromScene/mapToScene for mouse coordinates is probably incorrect, should
  map to scene and there to viewport??? But these methods are only used with
  filters, which are not in use currently anyway.
...
/**
 * Maps a point in global screen coordinates coordinates to item coordinates.
 *
 * The point is first converted from global coordinates to viewport
 * coordinates, then from viewport to scene coordinates, and finally, from
 * scene to item coordinates.
 */
static
QPointF globalPointMapToItem(const QPoint globalPoint, const QGraphicsItem *item)
{
    QGraphicsView *focusedGraphicsView =
        dynamic_cast<QGraphicsView *>(QApplication::focusWidget());
    if (!focusedGraphicsView) {
        // qWarning() << "No view in" << __FUNCTION__;
        // FIXME: in meego-terminal focusedGraphicsView is 0 at start-up,
        // called from paintFilters, see if a real problem when and if filters
        // are used
        return item->mapFromScene(globalPoint);
    }

    QPointF result;

    result = item->mapFromScene(
        focusedGraphicsView->mapToScene(
            focusedGraphicsView->mapFromGlobal(globalPoint)));

    return result;
}

/**
 * Returns the position of the cursor in item's coordinates.
 */
static inline
QPointF cursorPosMapToItem(const QGraphicsItem *item)
{
    return globalPointMapToItem(QCursor::pos(), item);
}
...
void TerminalDisplay::setColorTable(const ColorEntry table[])
{
  ...
  p.setColor( QPalette::Window, _colorTable[DEFAULT_BACK_COLOR].color );
  setPalette( p );
...
void TerminalDisplay::setVTFont(const QFont& f)
{
  ...
  // If based on QWidget the default size is set, hence the fist invocation of
  // this method via QTermWidget::init actually sets the fonts size. If based
  // in QGraphicsWidget the default size is not set, so font is not set, then
  // later on enormous _image is created since _fontWidth and _fontHeight are
  // both 1. Hence if default size is not set still allow setting the font:
  bool defaultSizeNotSet = (size().height() == 0 && size().width() == 0);

  if (defaultSizeNotSet ||
      ( metrics.height() < size().height() && metrics.maxWidth() < size().width() ))
  {
...
TerminalDisplay::TerminalDisplay(QGraphicsWidget *parent)
...
#if 0 // not used with Graphics View Framework
...
#if 0 // not used with Graphics View Framework, both attributes are unsupported by QGraphicsWidget, and autoFillBackground is false by default
...
{
	_blinkTimer -> disconnect();
	_blinkCursorTimer -> disconnect();
...
void TerminalDisplay::drawBackground(QPainter& painter, const QRectF& rect, const QColor& backgroundColor, bool useOpacitySetting )
{
...
void TerminalDisplay::processFilters() 
{
	...
	QRegion final = preUpdateHotSpots | postUpdateHotSpots;

	update( final.boundingRect() );
...
void TerminalDisplay::updateImage() 
{
  ...
  QPointF tL  = contentsRect().topLeft();
  ...
  dirtyRegion |= _inputMethodData.previousPreeditRect.toRect();
...
void  TerminalDisplay::paint ( QPainter * painter,
			       ...
			       QWidget * /* widget */)
{
    // default painter is from QGraphicsView and does not have needed font
...
//qDebug("%s %d paintEvent %d %d", __FILE__, __LINE__, paint.window().top(), paint.window().right());
    QPainter& paint = *painter; // keep QGraphicsWidget mods as little as possible
...
QRectF TerminalDisplay::preeditRect() const
{
...
void TerminalDisplay::drawInputMethodPreeditString(QPainter& painter , const QRectF& rect)
{
...
//qDebug("%s %d paintFilters", __FILE__, __LINE__);
    ...
    // lines for filters
    QPointF cursorPos = cursorPosMapToItem(this);
    int cursorLine;
    int cursorColumn;
    ...
    while (iter.hasNext())
    {
        ...
        for ( int line = spot->startLine() ; line <= spot->endLine() ; line++ )
        {
            ...
            if ( spot->type() == Filter::HotSpot::Link )
            {
                ...
                if ( r.contains( cursorPosMapToItem(this).toPoint() ) )
...
void TerminalDisplay::drawContents(QPainter &paint, const QRectF &rect)
{
...
//qDebug("%d %d %d %d", tLx, tLy, _contentWidth, _usedColumns * _fontWidth);  
  ...
  int lux = qMin(qreal(_usedColumns-1), qMax(zero,(rect.left()   - tLx - _leftMargin ) / _fontWidth));
  int luy = qMin(qreal(_usedLines-1),  qMax(zero, (rect.top()    - tLy - _topMargin  ) / _fontHeight));
  int rlx = qMin(qreal(_usedColumns-1), qMax(zero, (rect.right()  - tLx - _leftMargin ) / _fontWidth));
  int rly = qMin(qreal(_usedLines-1),  qMax(zero, (rect.bottom() - tLy - _topMargin  ) / _fontHeight));
...
void TerminalDisplay::resizeEvent(QGraphicsSceneResizeEvent * event)
{
  Q_UNUSED(event);
  updateImageSize();
}
...
void TerminalDisplay::propagateSize()
{
  if (_isFixedSize)
  {
     ...
     // TODO: QGraphicsWidget setFixedSize equivalent???
...
/**
 * Inspired by scrollBarPositionChanged
 */
void TerminalDisplay::viewportPositionChanged(int newCurrentLine, bool atEndOfOutput)
{
    _screenWindow->scrollTo(newCurrentLine);
...
void TerminalDisplay::scrollBarPositionChanged(int)
{
  ...
  // TODO:  Graphics View Framework equivalent
...
{
    emit scrollChanged(cursor, slines - _lines);
...
void TerminalDisplay::mousePressEvent(QGraphicsSceneMouseEvent* ev)
{
...
void TerminalDisplay::mouseMoveEvent(QGraphicsSceneMouseEvent* ev)
{
  int charLine = 0;
  int charColumn = 0;
...
void TerminalDisplay::extendSelection( const QPointF& position )
{
  if ( !_screenWindow )
      return;

  //if ( !contentsRect().contains(ev->pos()) ) return;
  const QPointF tL  = contentsRect().topLeft();
  const int     tLx = tL.x();
  const int     tLy = tL.y();
  ...
  QRectF textBounds(tLx + _leftMargin,
                    tLy + _topMargin,
  ...
  bool update = true;
  if ( oldpos.y() > textBounds.bottom() ) // scrollforward
  {
      linesBeyondWidget = (oldpos.y()-textBounds.bottom()) / _fontHeight;
      // _scrollBar->setValue(_scrollBar->value()+linesBeyondWidget+1);
      _screenWindow->scrollBy( ScreenWindow::ScrollLines , linesBeyondWidget + 1 );
  }
  else if ( oldpos.y() < textBounds.top() ) // scrollback
  {
...
void TerminalDisplay::mouseReleaseEvent(QGraphicsSceneMouseEvent* ev)
{
    if ( !_screenWindow )
        return;
...
void TerminalDisplay::mouseDoubleClickEvent(QGraphicsSceneMouseEvent* ev)
{
...
void TerminalDisplay::mouseTripleClickEvent(QGraphicsSceneMouseEvent* ev)
{
  if ( !_screenWindow ) return;
...
//qDebug("%s %d keyPressEvent and key is %d", __FILE__, __LINE__, event->key());
    ...
    // Ctrl and +/- change font size
    if (event->modifiers() & Qt::ControlModifier) {
        emitKeyPressSignal = false; // don't print +/- when changing text size
    ...
    // similar to upstream konsole in SessionController::trackOutput
...
// Graphics View Framework: this slows down text entry with hw keyboard (cursor
// movement becomes lazy)
#if 0 
...
bool TerminalDisplay::event( QEvent *e )
{
  ...
  // With the Graphics View Framework pressing Tab causes focusNextPrevChild to
  // be called and keyPressEvent is never called. Since Tab is essential for
  // shell completion, here default behavior is overridden to call
  // keyPressEvent, which also means focusNextPrevChild will not be called.
...
void TerminalDisplay::setFixedSize(int cols, int lins)
{
  ...
  // TODO: QGraphicsWidget setFixedSize equivalent???
  //QGraphicsWidget::setFixedSize(_size);
}
...
void TerminalDisplay::increaseTextSize()
{
    static const qreal MaxmimumFontSize = 200;

    QFont f = font();
    qreal newSize = qMin(f.pointSizeF() + 1, MaxmimumFontSize);
    f.setPointSizeF(newSize);
    setVTFont(f);
}

void TerminalDisplay::decreaseTextSize()
{
    static const qreal MinimumFontSize = 6;

    QFont f = font();
    qreal newSize = qMax(f.pointSizeF() - 1, MinimumFontSize);
    f.setPointSizeF(newSize);
    setVTFont(f);
}

AutoScrollHandler::AutoScrollHandler(QGraphicsWidget* parent)
: QObject(parent)
, _timerId(0)
{
...
inline
QGraphicsWidget* AutoScrollHandler::widget() const
{
    QGraphicsWidget* widget = dynamic_cast<QGraphicsWidget*>(parent());
    return widget;
}

inline
TerminalDisplay* AutoScrollHandler::terminalDisplay() const
{
    TerminalDisplay* display = dynamic_cast<TerminalDisplay*>(parent());
    return display;
}

/**
 * Terminal for touch screen devices runs normally in fullscreen mode. This
 * means mouse cannot move out of the widget, which is needed in konsole to
 * start auto-scrolling. So this and the following method return the margins
 * within the widget where auto-scrolling should start. Magic number 5 is due
 * to QCursor::pos() being always by about QPoint(3,3) greater than
 * mouseEvent->pos(). It is a mystery why, because with QWidget this does not
 * seem to occur.
 */
inline
qreal AutoScrollHandler::scrollBeginTopY() const
{
    int fontHeight = terminalDisplay() ? terminalDisplay()->fontHeight() : 0;
    return widget()->geometry().top() + fontHeight + 5;
}

inline
qreal AutoScrollHandler::scrollBeginBottomY() const
{
    int fontHeight = terminalDisplay() ? terminalDisplay()->fontHeight() : 0;
    return widget()->geometry().bottom() - fontHeight - 5;
}
...
void AutoScrollHandler::timerEvent(QTimerEvent* event)
{
    ...
    QPointF pos = cursorPosMapToItem(widget());
    bool mouseInWidget = widget()->rect().contains(pos);

    // don't modify if mouse is out of widget, let linesBeyondWidget do magic
    if (mouseInWidget && terminalDisplay()) {
        if (pos.y() <= scrollBeginTopY()) {
            pos.setY(widget()->geometry().top() - 1);
        }
        else if (pos.y() >= scrollBeginBottomY()) {
            pos.setY(widget()->geometry().bottom() + 1);
        }
    }
    ...
    widget()->scene()->sendEvent(widget(), &mouseEvent);
}
...
bool AutoScrollHandler::eventFilter(QObject* watched,QEvent* event)
{
    ...
    switch ( event->type() )
    {
        ...
        {
            bool mouseInWidget = mouseEvent->pos().y() > scrollBeginTopY() &&
                mouseEvent->pos().y() < scrollBeginBottomY();
        ...
        {
            if ( _timerId && (!mouseEvent->buttons() ||
