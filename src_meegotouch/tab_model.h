...
namespace karin
{
	...
	class tab_model : public QAbstractListModel
	{
		...
		public:
			...
			void addTab(qint64 id, const QString &title)
			{
				insertTab(id, title, tabList.size());
			}
			void removeTab(qint64 id);
			void updateTab(qint64 id, const QString &title);
			bool hasTab(qint64 id) const
			{
				return tabList.contains(id);
			}
			int indexTab(qint64 id) const
			{
				return tabList.indexOf(id);
			}
			qint64 valueTab(int i) const;

		private:
			QList<qint64> tabList;
			QMap<qint64, QString> tabMap;
