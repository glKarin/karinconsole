...
#else
#define IM_TOOLBARS_DIR "/src_meegotouch/toolbars/"
#endif
...
class Toolbar;
typedef QList<Toolbar*> Toolbars_t;

struct Toolbar
{
	Toolbar(QString fileName);
	~Toolbar();
	// the registered unique identifier of this toolbar as returned by
	// MInputMethodState
	int id;
	QString fileName;
	QString name; // pretty name for the UI
	static const QStringList & getAllToolbars(bool forceUpdate = false);
...
public:
    ...
    Toolbars_t toolbars() const { return Toolbar::getAllToolbars(); }
		*/
    const Toolbar* activeToolbar() const // returns 0 if there are no toolbars
        { return m_toolbar; }
